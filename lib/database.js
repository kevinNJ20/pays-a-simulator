...
class PaysADatabase {
  ...
  // ✅ AJOUT: Méthode pour détecter le format du manifeste
  ...
  // ✅ AJOUT: Méthode de conversion legacy vers UEMOA
  convertirLegacyVersUEMOA(donneesLegacy, id) {
    return {
      id,
      format: 'UEMOA',
      numero_manif: Date.now(),
      annee_manif: new Date().getFullYear().toString(),
      bureau_manif: '18N',
      consignataire: donneesLegacy.transporteur || 'UNKNOWN',
      navire: donneesLegacy.navire || 'UNKNOWN',
      provenance: donneesLegacy.portEmbarquement || 'UNKNOWN',
      date_arrivee: donneesLegacy.dateArrivee || new Date().toISOString().split('T')[0],
      statut: 'MANIFESTE_CREE',
      dateCreation: new Date(),
      paysOrigine: this.paysInfo.code,
      portDebarquement: this.paysInfo.port,
      etapeWorkflow: 'CREATION_MANIFESTE',
      transmissionKit: null,
      nbre_article: donneesLegacy.marchandises?.length || 0,
      articles: (donneesLegacy.marchandises || []).map((m, index) => ({
        art: index + 1,
        pays_dest: m.paysDestination || 'MALI',
        ville_dest: 'BAMAKO',
        marchandise: m.designation || 'Marchandise',
        poids: m.poidsBrut || 0,
        destinataire: m.destinataire || 'UNKNOWN',
        connaissement: ''
      })),
      workflow: {
        etape1_manifesteRecu: new Date(),
...
creerDeclarationTransit(donneesTransit) {
  console.log(`🚛 [${this.paysInfo.code}] TRANSIT ÉTAPES 1-9: Création déclaration transit`);
  ...
  const declarationTransit = {
    ...
    // ✅ CORRECTION: Renseigner les étapes 7-9 automatiquement
    etapesTransit: {
      ...
      // ✅ NOUVEAU: Étapes 7-9 renseignées automatiquement
      etape7_garanties: new Date(),           // Octroi garanties (caution)
      etape8_paiement: new Date(),            // Paiement redevances
      etape9_bonEnlever: new Date(),          // Bon à enlever / Autorisation départ
      etape10_debut: new Date(),              // Début transit
      
      // Étapes à compléter plus tard
      etape14_messageArrivee: null,
      etape15_apurement: null,
      etape16_liberationGaranties: null
    },
    workflow: 'TRANSIT'
  };
  ...
  console.log(`✅ [${this.paysInfo.code}] TRANSIT ÉTAPES 1-9: Déclaration ${transitId} créée et autorisée`);
  this.ajouterInteractionWorkflow('TRANSIT_CREE', 
    `Transit étapes 1-9: Déclaration ${transitId} créée, garanties prises, redevances payées, bon à enlever émis`);
  ...
  obtenirInteractionsKit(limite = 10) {
