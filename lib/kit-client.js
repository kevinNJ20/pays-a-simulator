...
class KitInterconnexionClient {
  ...
  setupInterceptors() {
    ...
    this.client.interceptors.request.use(
      (config) => {
        ...
        console.log(`📤 [${this.paysCode}] Headers:`, JSON.stringify(config.headers, null, 2));
        console.log(`📤 [${this.paysCode}] Payload:`, JSON.stringify(config.data, null, 2));
        return config;
      },
      (error) => {
    ...
    this.client.interceptors.response.use(
      (response) => {
        ...
        console.log(`📥 [${this.paysCode}] Réponse Data:`, JSON.stringify(response.data, null, 2));
  ...
  // === TRANSMISSION DE MANIFESTE CORRIGÉE ===
  async transmettreManifeste(manifeste) {
    try {
      ...
      // ✅ CORRECTION: Préparer le payload selon le format attendu par le Kit
      const manifesteForKit = {
        ...
        marchandises: manifeste.marchandises.map(marchandise => ({
          codeSH: marchandise.codeSH,
          designation: marchandise.designation,
          poidsBrut: marchandise.poidsBrut,
          nombreColis: marchandise.nombreColis,
          destinataire: marchandise.destinataire,
          paysDestination: marchandise.paysDestination
        }))
      };

      console.log(`📦 [${this.paysCode}] Payload préparé pour Kit:`, JSON.stringify(manifesteForKit, null, 2));
      ...
      console.error(`❌ [${this.paysCode}] Échec transmission manifeste:`, {
        message: error.message,
        status: error.response?.status,
        statusText: error.response?.statusText,
        responseData: error.response?.data,
      ...
      // ✅ CORRECTION: Throw avec plus de détails
      const errorMessage = error.response?.data?.message || 
                          error.response?.statusText || 
                          error.message;
      throw new Error(`Transmission manifeste échouée: ${errorMessage} (Status: ${error.response?.status || 'Network Error'})`);
    }
  }
