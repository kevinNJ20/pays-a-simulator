...
class KitInterconnexionClient {
  ...
  validerManifeste(manifeste) {
    ...
    console.log(`🔍 [PAYS A] Format détecté: ${format}`);
    ...
    if (format === 'UEMOA') {
      ...
      // Validation numero_manif
      if (!manifeste.numero_manif) {
        console.error('❌ [PAYS A] numero_manif manquant:', {
          numero_manif: manifeste.numero_manif,
      ...
      // Validation détaillée de chaque article
  ...
  detecterFormatManifeste(manifeste) {
    if (!manifeste || typeof manifeste !== 'object') return 'UNKNOWN';
    ...
    const hasLegacyFields = champsLegacy.some(champ => manifeste.hasOwnProperty(champ));
