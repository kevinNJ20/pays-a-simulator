...
class KitInterconnexionClient {
  constructor() {
    ...
    this.client = axios.create({
      ...
      headers: {
        ...
        'X-Manifeste-Format': 'UEMOA', // ✅ AJOUT: Header requis par MuleSoft
  ...
  setupInterceptors() {
    this.client.interceptors.request.use(
      (config) => {
        ...
        // ✅ AJOUT: S'assurer que X-Manifeste-Format est toujours présent
    ...
    this.client.interceptors.response.use(
      ...
      (error) => {
        ...
        // ✅ AJOUT: Afficher les détails de l'erreur MuleSoft si disponibles
  ...
  async transmettreManifeste(manifeste) {
    try {
      ...
      // Validation du manifeste Sénégal
      this.validerManifesteSenegal(manifeste);
      
      // Préparation extraction pour pays de destination
      ...
      // ✅ CORRECTION: Ajouter explicitement les headers requis par MuleSoft
      const response = await this.client.post('/manifeste/transmission', extractionManifeste, {
        headers: {
          ...
          'X-Manifeste-Format': 'UEMOA', // ✅ Header requis
      ...
      // ✅ AJOUT: Détails spécifiques si erreur MuleSoft
      if (error.response?.data) {
      ...
      enrichedError.muleSoftError = error.response?.data; // ✅ AJOUT
  ...
  validerManifesteSenegal(manifeste) {
    ...
    if (format === 'UEMOA') {
      ...
      // Vérifier qu'il y a des marchandises pour pays de destination
  ...
  preparerExtractionPourDestination(manifeste) {
    ...
    // Extraire uniquement les articles destinés aux pays de l'hinterland
    ...
    const extraction = {
      // ✅ Informations manifeste origine (Sénégal)
      ...
      // ✅ Informations Sénégal (pays de prime abord)
      paysOrigine: 'SENEGAL',
      ...
      etapeWorkflow: 5, // Étape 5 : Extraction transmise
  ...
  // ✅ Notification apurement (étape 20+)
